    ...
    form.verify({
        ...
        , nowdate: function (value, item) {
            ...
            if (start > end) {
                setTimeout(() => {
                    $("#birthday").blur()
                    $("#birthday").css("border-color","#ff5722")
                },100)
                return '所选日期大于当前时间';
            }
        }
        ...
        , score: function (value, item) {
            if (parseInt(value) > parseInt($("#MaxScore").val())) {
                return '最低分数不能大于最高分数';
            }
        }
        ...
        , parentIdrepuired: function() {
          return ($("#parentId").value, document.querySelectorAll(".layui-unselect")[1], 'parentIdrepuired')
          if($("#parentId").value == ''){
            setTimeout(function(){
              document.querySelectorAll(".layui-unselect")[1].css("border-color", "#FF5722")
            },200)
          }
        }
    });
