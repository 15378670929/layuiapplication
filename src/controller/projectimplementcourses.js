                        ...
                        common.ajax(setter.apiAddress.coursecode.checkmodel, "GET", "", {
                            ...
                            if (res.statusCode == 200) {
                                ...
                                if(coursecode.code == ''){
                                    $("#effectiveDate").val("");
                                    $("#expirationDate").val('');
                                } else if (new Date(coursecode.expirationDate.replace("-", "/").replace("-", "/")) <= new Date(now.replace("-", "/").replace("-", "/"))) {
                                ...
                                $("#coursecode").click(function () {
                                    ...
                                    $("#effectiveDate").val("课程码在" + dayss + "天 0小时 0 分钟0 秒之后过期");
                                });
                                        ...
                                        layer.confirm('一共选择' + checkStatus.data.length + '个用户，确定删除？', function (index) {
                                            ...
                                            for (var i = 0; i < userconfigdata.length; i++) {
                                                models.push({ 
                                                  ...
                                                  organizationId: userconfigdata[i].organizationId,  
                                                  ProjectId: userconfigdata[i].projectId,
                                                  CourseId: userconfigdata[i].courseId,
                                                  CourseChildId: userconfigdata[i].courseChildId,
                                                  UserId: userconfigdata[i].userId,
                                                  userName: userconfigdata[i].userName
                                                });
                                            }
                                    ...
                                    if (obj.event === 'userdel') {
                                        layer.confirm('确定删除？', function (index) {
                                            if (rowuserdata.id != "") {
                                                ...
                                                models.push({ 
                                                  Id: rowuserdata.id,
                                                  organizationId: rowuserdata.organizationId,  
                                                  ProjectId: rowuserdata.projectId,
                                                  CourseId: rowuserdata.courseId,
                                                  CourseChildId: rowuserdata.courseChildId,
