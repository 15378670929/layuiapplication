...
layui.extend({
    ...
    view: 'lib/view', //核心模块
    oidcconfig: 'controller/oidcconfig',
    common: 'controller/common',
}).define(['setter', 'admin', 'oidcconfig', 'table', 'common'], function (exports) {
    var setter = layui.setter
        ...
        , renderPage = function () {
            ...
            view().render(path.join('/')).then(function (res) {
                //遍历页签选项卡
                var matchTo
                    , tabs = $('#IQuizoo_app_tabsheader>li');
        ...
        , entryPage = function (fn) {
            ...
            if (isIndPage) {
                ...
                if (setter.interceptor) {
                    oidcconfig.getUser().then(function (user) {
                        if (user) {
                            sessionStorage.setItem("username", user.profile.username);
                            sessionStorage.setItem("access_token", user.access_token);
                            sessionStorage.setItem("roleid", user.profile.role);
                            ...
                            if (admin.pageType === 'console') {
                                ...
                                //初始控制台结构
                                common.ajax(setter.apiAddress.clientmodules.roletreelist, "GET", "", { roleId: user.profile.role }, function (res) {
                                    var data = res.data
                            ...
                            oidcconfig.signinRedirect();
                        }
                    });
                }
            }
        }
        , APP_BODY = '#IQuizoo_app_body', FILTER_TAB_TBAS = 'layadmin-layout-tabs'
        , $ = layui.$, $win = $(window);
    //初始主体结构
    layui.link(
