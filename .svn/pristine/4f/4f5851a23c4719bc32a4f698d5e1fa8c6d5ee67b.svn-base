    ...
    table.on('tool(questions-group-table)', function (obj) {
        ...
        if (obj.event === 'changestatus') {
                ...
                , success: function (layero, index) {
                    view(this.id).render('content/questiongroup/edit', data).done(function () {
                        ...
                        common.ajax(setter.apiAddress.questiongrouptemplate.loadall, "GET", "", {}, function (res) {
                            common.select("grouptemplateId", res.data, data.grouptemplateId, 1);
                            form.render("select");
                        });
                        //获取算分码
                        ...
                        //获取顶级能力
                        common.ajax(setter.apiAddress.ability.selname, "GET", "", { level: 0 }, function (res) {
                            common.select("abilityType", res.data, data.abilityTypeId, 1);
                            //获取一级能力
                            common.ajax(setter.apiAddress.ability.selname, "GET", "", { pid: data.abilityTypeId }, function (res) {
                                common.select("firstAbilities", res.data, data.firstAbilityId, 2);
                                form.render("select");
                            });
                            //获取二级能力
                            common.ajax(setter.apiAddress.ability.selname, "GET", "", { pid: data.firstAbilityId }, function (res) {
                                common.select("secondAbilities", res.data, data.secondAbilityId, 3);
                                form.render("select");
                            });
                            form.render("select");
                        });
                        ...
                        $("input[name=GroupModel][value=1]").attr("checked", data.groupModel == "1" ? true : false);
                        $("input[name=GroupModel][value=2]").attr("checked", data.groupModel == "2" ? true : false);
            ...
            if (null !== rowdata.questionGroupTemplateDto) {
                window.open(rowdata.questionGroupTemplateDto.url + '?questionGroupId='+ rowdata.id + '&url=' + setter.apiurl);
            } else {
                layer.msg("未选择模板");
            }
        }
